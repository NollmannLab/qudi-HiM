...
class FocusLogic(GenericLogic):
    ...
    def on_activate(self):
        ...
        # check if there is a shutter for the IR laser (only for the RAMM setup)
        if self._laser_shutter:
            self._laser_shutter_initialize = self._autofocus_logic.initialize_shutter()
    ...
    def on_deactivate(self):
        ...
        if self._laser_shutter:
            self._autofocus_logic.close_laser_shutter()
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    def start_live_display(self):
        ...
        if self._laser_shutter:
            if self.check_laser_shutter():
                return
            else:
                self._autofocus_logic.open_laser_shutter()
...
# ----------------------------------------------------------------------------------------------------------------------
    ...
    def calibrate_focus_stabilization(self):
        ...
        if self._laser_shutter and not self.live_display_enabled:
            self._autofocus_logic.close_laser_shutter()
...
# ----------------------------------------------------------------------------------------------------------------------
# Helper functions to handle the shutter for the IR laser on the RAMM setup
# ----------------------------------------------------------------------------------------------------------------------

    def check_laser_shutter(self):
        if self._laser_shutter_initialize:
            abort = False
        else:
            self.log.warning('Shutter was not properly initialized - action is aborted')
            abort = True
        return abort
